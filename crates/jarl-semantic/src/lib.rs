...
impl<'a> SemanticInfo<'a> {
    ...
    fn collect_branching_argument_ranges(&mut self, args: &[RArgument]) {
        if args.len() < 2 {
            return;
        }
        ...
        self.branching_args.extend(
            args.iter()
                ...
                .map(|value| (group, value.syntax().text_trimmed_range())),
        );
    }
    ...
    fn visit_call(&mut self, call: &RCall) {
        ...
        self.collect_branching_argument_ranges(&args);
    ...
    /// [`Self::resolve_assignment_groups`]). Only definitions
