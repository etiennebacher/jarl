...
pub fn parse_flir_toml(path: &Path) -> Result<TomlOptions, ParseTomlError> {
...
/// Return the path to the `flir.toml` or `.flir.toml` file in a given directory.
pub fn find_flir_toml_in_directory<P: AsRef<Path>>(path: P) -> Option<PathBuf> {
    // Check for `flir.toml` first, as we prioritize the "visible" one.
    let toml = path.as_ref().join("flir.toml");
    if toml.is_file() {
        return Some(toml);
    }

    // Now check for `.flir.toml` as well
    let toml = path.as_ref().join(".flir.toml");
    if toml.is_file() {
        return Some(toml);
    }
...
/// Find the path to the closest `flir.toml` or `.flir.toml` if one exists, walking up the filesystem
pub fn find_flir_toml<P: AsRef<Path>>(path: P) -> Option<PathBuf> {
    for directory in path.as_ref().ancestors() {
        if let Some(toml) = find_flir_toml_in_directory(directory) {
            return Some(toml);
        }
    }
    None
