...
pub struct PackageAnalysis {
    ...
    /// exported.
    pub unused_internal_functions: HashMap<PathBuf, Vec<(String, TextRange, String)>>,
}
...
pub fn compute_package_analysis(paths: &[PathBuf], config: &Config) -> PackageAnalysis {
    ...
    let unused_internal_functions = if config
        .rules_to_apply
        .contains(&Rule::UnusedInternalFunction)
    {
        compute_package_unused_internal_functions(paths)
    } else {
        HashMap::new()
    };

    PackageAnalysis { duplicate_assignments, unused_internal_functions }
}
