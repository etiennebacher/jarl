...
pub fn call(r_expr: &RCall, checker: &mut Checker) -> anyhow::Result<()> {
    ...
    if checker.is_rule_enabled("fixed_regex") && !checker.should_skip_rule(node, "fixed_regex") {
    ...
    if checker.is_rule_enabled("sprintf") && !checker.should_skip_rule(node, "sprintf") {
