...
/// 3. Check if the language ID matches your target language, in which case we include it.
///    This is a feature unique to language servers.
...
fn is_document_excluded(
    ...
    // Then check if the language id matches your target language, which is a feature unique to LSPs
    if language_id == "flir2" || language_id == "plaintext" {
        tracing::trace!(
            "Included file due to client provided language id '{language_id}' {path}",
            path = path.display()
        );
        return false;
    }
