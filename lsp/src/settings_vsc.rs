...
impl VscDiagnosticsSettings {
    pub(crate) fn section_from_key(key: &str) -> &str {
        match key {
            "enable" => "flir2.diagnostics.enable",
            _ => "unknown", // To be caught via downstream errors
        }
    }
}
...
impl VscGlobalSettings {
    pub(crate) fn section_from_key(key: &str) -> &str {
        match key {
            "log_level" => "flir2.logLevel",
            "dependency_log_levels" => "flir2.dependencyLogLevels",
            "sync_file_settings_with_client" => "flir2.syncFileSettingsWithClient",
            _ => "unknown", // To be caught via downstream errors
        }
    }
}
